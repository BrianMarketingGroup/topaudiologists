...
const BENEFITS = [
  { icon: TrendingUp, title: "Increase Your Visibility", body: "Help patients searching for hearing and balance care discover your practice." },
  { icon: ShieldCheck, title: "Build Trust & Credibility", body: "A professional profile highlights your credentials, experience, and hearing care expertise." },
  { icon: Star, title: "Differentiate Your Practice", body: "Highlight your diagnostic services, hearing technology, and patient-centered care." },
  { icon: Users, title: "Reach Patients at Every Stage", body: "Patients often compare providers before selecting an audiologist for hearing evaluations and treatment." },
  { icon: Zap, title: "Highlight Your Expertise", body: "Whether you specialize in hearing aids, cochlear implants, tinnitus management, pediatric audiology, balance disorders, or diagnostic evaluations, your profile helps communicate your expertise." },
...
const HOW_IT_WORKS = [
  { step: "01", title: "Submit Your Application", body: "Complete our simple application with your practice details, audiology specialties, and the cities where you want to be listed." },
  { step: "02", title: "Your Profile Goes Live", body: "We build and publish your professional practice profile, optimized to appear when patients search for hearing care in your city." },
...
const SPECIALTY_TYPES = [
  "Hearing Aids", "Cochlear Implants", "Tinnitus Management",
  "Pediatric Audiology", "Balance Disorders", "Diagnostic Evaluations",
];
...
const EVAL_CRITERIA = [
  ...
  { icon: Zap, label: "Hearing Technology" },
...
function EyebrowDark({ children, tone = "teal" }: { children: React.ReactNode; tone?: "teal" | "gold" }) {
  return (
    <p className={`inline-flex items-center gap-2.5 text-[11px] font-semibold tracking-[0.3em] uppercase mb-5 ${tone === "gold" ? "text-gold-light" : "text-teal-light"}`}>
      <span className={`h-1.5 w-1.5 rotate-45 ${tone === "gold" ? "bg-gold-light" : "bg-teal-light shadow-[0_0_8px_2px_rgba(129, 140, 248,0.6)]"}`} />
      {children}
    </p>
  );
}
...
function EyebrowLight({ children, tone = "teal" }: { children: React.ReactNode; tone?: "teal" | "gold" }) {
  return (
    <p className={`inline-flex items-center gap-2.5 text-[11px] font-semibold tracking-[0.3em] uppercase mb-5 ${tone === "gold" ? "text-gold-dark" : "text-teal"}`}>
      <span className={`h-1.5 w-1.5 rotate-45 ${tone === "gold" ? "bg-gold" : "bg-teal shadow-[0_0_8px_2px_rgba(79, 70, 229,0.4)]"}`} />
      {children}
    </p>
  );
}
          ...
          <FadeIn variant="luxe">
            <div className="text-center mb-16 max-w-2xl mx-auto">
              ...
              <p className="text-slate-500 text-lg">
                Patients compare experience, technology, hearing solutions, and treatment options before choosing an audiologist.
        ...
        <Container className="relative">
          <FadeIn variant="luxe">
            <div className="text-center mb-10 sm:mb-14 max-w-2xl mx-auto">
              ...
              <h2 className="race-head text-3xl sm:text-5xl text-pearl mb-4">How Patients Choose an Audiologist</h2>
              <p className="text-pearl/60 text-base sm:text-lg">
                ...
                before committing to a hearing care practice.
              </p>
            </div>
          </FadeIn>
            ...
            <FadeIn delay={0.15} variant="luxe">
              <div className="bg-white border border-slate-200 rounded-2xl p-8 hover:-translate-y-1.5 hover:shadow-xl hover:shadow-teal/8 transition-all duration-300 h-full">
                ...
                <ul className="space-y-3 text-sm text-slate-600 mb-8">
                  {["Dedicated practice profile page", "Audiology specialties & services", "City directory placement", "Provider credentials & contact info", "Treatment philosophy & description"].map((f) => (
