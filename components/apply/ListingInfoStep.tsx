      ...
      {completeLater ? (
        ...
        <>
          ...
          <FormField label="Business Name" required error={errors.businessName?.message}>
            <Input {...register("businessName")} error={errors.businessName?.message} placeholder="e.g. Denver Hearing & Balance Care" />
          </FormField>

          {/* Audiologists / Providers */}
          <div className="space-y-4">
            <div>
              <p className="text-sm font-semibold text-navy">Audiologist(s) / Provider(s)</p>
              ...
              <div key={field.id} className="p-4 rounded-xl border border-pearl-dark bg-pearl relative">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4 pr-8">
                  <FormField label="Name" required error={errors.providers?.[index]?.name?.message}>
                    <Input {...register(`providers.${index}.name` as const)} placeholder="e.g. Dr. Maria Chen, Au.D." error={errors.providers?.[index]?.name?.message} />
                  </FormField>
                  <FormField label="Title / Role" hint="Optional">
                    <Input {...register(`providers.${index}.title` as const)} placeholder="e.g. Lead Audiologist, Clinical Director" />
                  </FormField>
                </div>
                <FormField label="Bio / Credentials" hint="Optional">
                  <Textarea {...register(`providers.${index}.description` as const)} placeholder="Au.D., 15+ years fitting hearing aids, tinnitus and balance specialist…" rows={2} />
                </FormField>
              </div>
            ))}
